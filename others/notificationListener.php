...
function send_notification_mail($notifyId,$notification_array)
{
	...
	{	
		...
		$following_un=	$notification_array['following_username'];
		$following_username = getUserFullNamefromUserName($following_un);
		
		
		$followed_un = $notification_array['followed_username'];
		$followed_username = getUserFullNamefromUserName($followed_un);
		$to_email = $followed_email = $notification_array['followed_email'];	
	}
	//To send comment notification email	
	elseif($type === 'comment')
	{		
		$commentAuthorUN = $notification_array['commentAuthorUsername'];
		$commentAuthorUsername = getUserFullNamefromUserName($commentAuthorUN);
		
		$postAuthorUN = $notification_array['postAuthorUsername'];
		$postAuthorUsername = getUserFullNamefromUserName($postAuthorUN);
	...
	elseif($type === 'like')
	{		
		$likerUN = $notification_array['likerUsername'];
		$likerUsername = getUserFullNamefromUserName($likerUN);
...
//Function to fetch user's full name using username
function getUserFullNamefromUserName($userName)
{
	
	$qry="SELECT firstname,lastname FROM entrp_login  
